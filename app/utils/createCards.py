...
class Cards():
    ...
    def create_task(self, chat):
        with st.form(key='todo', clear_on_submit=True):

            text = st.text_input("Descreva seu projeto")
            submit_button = st.form_submit_button("Create")
            if submit_button == True:
             # connection.insert_todo(todo)
                chat.project_creator(text)

            # title = st.text_input("Insert the title of your card here")
            # task = st.text_input("Insert your card description here:")
    ...
    def create_card(self, task:list, user_id):
        ...
        task_display.subheader(task['title'])
        ...
        new_status = task_display.selectbox("Status", options, key=task['card_id'], index=options.index(task['status']))
        if new_status != task['status']:
            connection.update_todo(task['card_id'], "status", new_status)
            connection.get_data(user_id)
        
        tag_id = "_tag_multiselect_" + str(task['card_id'])
